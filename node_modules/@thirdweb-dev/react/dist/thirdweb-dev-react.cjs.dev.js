...
function useContract(contractAddress) {
  ...
  ["contract-instance", contractAddress, {
    ...
    invariant__default["default"](activeChainId, "active chain id is required"); // first fetch the contract type (we fetch this explicitly via the queryClient so **it** gets cached!)
    ...
    invariant__default["default"](cType, "could not get contract type"); // if the contract type is NOT "custom", we can use the built-in contract method from the SDK
    ...
    invariant__default["default"](compMetadata, "could not get compiler metadata"); // if we have the compiler metadata, we can use the custom contract
...
function convertResponseToNFTType(contract, metadata) {
  if (contract instanceof sdk.Erc721) {
    return {
      type: "ERC721",
      supply: 1,
      owner: "",
  ...
  if (contract instanceof sdk.Erc1155) {
    return {
      type: "ERC1155",
      supply: 0,
      owner: "",
...
function useNFT(contract, tokenId) {
  ...
  return useQueryWithNetwork(cacheKeys.contract.nft.get(contractAddress, tokenId), async () => {
    ...
    if (erc1155) {
      ...
      return convertResponseToNFTType(erc1155, await erc1155.get(ethers.BigNumber.from(tokenId || 0)));
    }

    if (erc721) {
      invariant__default["default"](erc721.get, "Contract instance does not support get");
      return convertResponseToNFTType(erc721, await erc721.get(ethers.BigNumber.from(tokenId || 0)));
    }
...
exports.convertResponseToNFTType = convertResponseToNFTType;
exports.convertResponseToNFTTypeArray = convertResponseToNFTTypeArray;
