import React, { useMemo, useContext, createContext, useState, useEffect, useRef, useLayoutEffect, useCallback, useId } from 'react';
import { jsx, jsxs, Fragment } from 'react/jsx-runtime';
import { ChainId, NATIVE_TOKENS, UserWallet, ThirdwebSDK, getProviderForNetwork, Erc721, Erc1155, ListingType, SUPPORTED_CHAIN_IDS } from '@thirdweb-dev/sdk';
...
function useContract(contractAddress) {
  ...
  ["contract-instance", contractAddress, {
    ...
    invariant(activeChainId, "active chain id is required"); // first fetch the contract type (we fetch this explicitly via the queryClient so **it** gets cached!)

    const cType = await queryClient.fetchQuery(contractType.cacheKey(contractAddress, activeChainId), () => contractType.fetchQuery(contractAddress, sdk), {
      cacheTime: Infinity,
      staleTime: Infinity
    }); // if we can't get the contract type, we need to exit

    invariant(cType, "could not get contract type"); // if the contract type is NOT "custom", we can use the built-in contract method from the SDK
    ...
    const compMetadata = await queryClient.fetchQuery(compilerMetadata.cacheKey(contractAddress, activeChainId), () => compilerMetadata.fetchQuery(contractAddress, sdk), {
      cacheTime: Infinity,
      staleTime: Infinity
    }); // if we can't get the compiler metadata, we need to exit

    invariant(compMetadata, "could not get compiler metadata"); // if we have the compiler metadata, we can use the custom contract
...
function useEditionDrop(contractAddress) {
  ...
  return useContract(contractAddress).contract;
}
...
function useNFT(contract, tokenId) {
  ...
  return useQueryWithNetwork(cacheKeys.contract.nft.get(contractAddress, tokenId), async () => {
    ...
    if (erc1155) {
      ...
      return convertResponseToNFTType(erc1155, await erc1155.get(BigNumber.from(tokenId || 0)));
    }

    if (erc721) {
      invariant(erc721.get, "Contract instance does not support get");
      return convertResponseToNFTType(erc721, await erc721.get(BigNumber.from(tokenId || 0)));
    }
...
function useNFTs(contract, queryParams) {
  ...
  return useQueryWithNetwork(cacheKeys.contract.nft.query.all(contractAddress, queryParams), async () => {
    ...
    if (erc721) {
      ...
      return convertResponseToNFTTypeArray(erc721, await erc721.getAll(queryParams));
    }

    if (erc1155) {
      invariant(erc1155.getAll, "Contract instance does not support getAll");
      return convertResponseToNFTTypeArray(erc1155, await erc1155.getAll(queryParams));
    }
...
function useOwnedNFTs(contract, ownerWalletAddress) {
  ...
  return useQueryWithNetwork(cacheKeys.contract.nft.query.owned.all(contractAddress, ownerWalletAddress), async () => {
    ...
    if (erc721) {
      return convertResponseToNFTTypeArray(erc721, await erc721.getOwned(ownerWalletAddress));
    }

    if (erc1155) {
      return convertResponseToNFTTypeArray(erc1155, await erc1155.getOwned(ownerWalletAddress));
    }
...
export { ConnectWallet, MediaRenderer, ThirdwebNftMedia, ThirdwebProvider, ThirdwebSDKProvider, Web3Button, WrappedThirdwebSDKProvider, compilerMetadata, contractType, convertResponseToNFTType, convertResponseToNFTTypeArray, getErc1155, getErc20, getErc721, getErcs, useAccount, useActiveClaimCondition, useActiveListings, useAddress, useAirdropNFT, useAllRoleMembers, useAuctionWinner, useAuth, useBalance, useBatchesToReveal, useBidBuffer, useBurnNFT, useBurnToken, useBuyNow, useCancelListing, useChainId, useClaimConditions, useClaimIneligibilityReasons, useClaimNFT, useClaimToken, useClaimedNFTSupply, useClaimedNFTs, useCoinbaseWallet, useCompilerMetadata, useConnect, useConnectedWallet, useContract, useContractEvents, useContractMetadata, useContractMetadataUpdate, useContractRead, useContractType, useContractWrite, useCreateAuctionListing, useCreateDirectListing, useDelayedRevealLazyMint, useDesiredChainId, useDisconnect, useEdition, useEditionDrop, useGnosis, useGrantRole, useIsAddressRole, useLazyMint, useListing, useListings, useListingsCount, useLogin, useLogout, useMagic, useMakeBid, useMarketplace, useMetadata, useMetamask, useMintNFT, useMintNFTSupply, useMintToken, useMultiwrap, useNFT, useNFTBalance, useNFTCollection, useNFTDrop, useNFTs, useNetwork, useNetworkMismatch, useOwnedNFTs, usePack, usePlatformFees, usePrimarySaleRecipient, useReadonlySDK, useResetClaimConditions, useResolvedMediaType, useRevealLazyMint, useRevokeRole, useRoleMembers, useRoyaltySettings, useSDK, useSDKChainId, useSetAllRoleMembers, useSetClaimConditions, useSignatureDrop, useSigner, useSplit, useToken, useTokenBalance, useTokenDecimals, useTokenDrop, useTokenSupply, useTotalCirculatingSupply, useTotalCount, useTransferBatchToken, useTransferNFT, useTransferToken, useUnclaimedNFTSupply, useUnclaimedNFTs, useUpdateMetadata, useUpdatePlatformFees, useUpdatePrimarySaleRecipient, useUpdateRoyaltySettings, useUser, useVote, useWalletConnect, useWalletLink, useWinningBid };
